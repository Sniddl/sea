...
sudo apt-get update
sudo apt-get install nginx git emacs g++ make openssl fcgiwrap -y
...
sudo cp /app/.vagrant_setup/nginx/self-signed.conf /etc/nginx/snippets/self-signed.conf
sudo cp /app/.vagrant_setup/nginx/ssl-params.conf /etc/nginx/snippets/ssl-params.conf
